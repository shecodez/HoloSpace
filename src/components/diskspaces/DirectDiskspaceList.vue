<template>
    ...
    <div class="collapse-content p-0">
      <ul class="diskspaces">
        <template v-for="space in diskspaces" :key="space.id">
          <DirectDiskspaceListItem :type="type" :diskspace="space" @edit="openModal(space)" />
        </template>
      </ul>
    </div>
  </div>
  ...
  <AddDiskspaceModal isDirect :isOpen="isAddDiskspace" :type="type" @close="closeModal" />
  <EditDiskspaceModal isDirect :isOpen="!!editDiskspace" :diskspace="editDiskspace" @close="closeModal" />
</template>
